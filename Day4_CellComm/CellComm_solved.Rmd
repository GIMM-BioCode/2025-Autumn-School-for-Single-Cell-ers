...
VlnPlot(srat, features = "HM_EMT1", group.by = "level3_celltype", 
        sort = T, split.by = "tum.norm")
```
...
```{r, fig.height=4, fig.width=12}
Idents(srat) = srat$level3_celltype
VlnPlot(srat, features = "HM_EMT1", group.by = "level3_celltype", split.by = "tum.norm", 
        idents = c("TSK", "KC_Basal", 
                   "KC_Cyc", "KC_Diff"), sort = T)
```
...
```{r}
Idents(srat) = srat$tum.norm
comp_l = list("Basal" = c("KC_Basal"),
              "Diff" = c("KC_Diff"),
              "TSK" = c("TSK", "KC_Basal"))
de_cond_l = list()
for(n in names(comp_l)){
  if(n!="TSK"){
    cells = srat$level3_celltype %in% comp_l[[n]]
    print(table(srat[,cells]$level3_celltype, srat[,cells]$tum.norm))
  } else{
    cells = srat$level3_celltype=="TSK" | 
      (srat$level3_celltype=="KC_Basal" & srat$tum.norm=="Normal")
    print(table(srat[,cells]$level3_celltype, srat[,cells]$tum.norm))
  }
  de_cond_l[[n]] = FindMarkers(srat[,cells], ident.1 = "Tumor", ident.2 = "Normal")
}

mk_clusters = de_cond_l$TSK
```
...
```{r}
srat_n = srat[,srat$tum.norm=="Normal"]
srat_t = srat[,srat$tum.norm=="Tumor"]
```
...
```{r}
cell_tab = table(srat_n$level3_celltype)
cells_keep = colnames(srat_n)[srat_n$level3_celltype %in% names(cell_tab)[cell_tab>=5] &
                                srat_n$level3_celltype!="Multiplet"]
srat_n = srat_n[,cells_keep]
print(sort(unique(srat_n$level3_celltype)))

cell_tab = table(srat_t$level3_celltype)
cells_keep = colnames(srat_t)[srat_t$level3_celltype %in% names(cell_tab)[cell_tab>=5] &
                                srat_t$level3_celltype!="Multiplet"]
srat_t = srat_t[,cells_keep]
print(sort(unique(srat_t$level3_celltype)))
```
...
print(dim(liana_n))
print(dim(liana_n_filt))
print(dim(liana_t))
print(dim(liana_t_filt))
```
...
```{r}
l_cond = data.frame(igraph::layout_in_circle(network_cond, order = names(sort(rowSums(mat_diff)))))
rownames(l_cond) = colnames(mat_diff)
l_cond$total = rowSums(mat_diff)
l_cond$name = rownames(l_cond)
...
pltboth = ggplot()+
  geom_segment(data = edge_df[abs(edge_df$Freq)>25,], 
               mapping = aes(x = x1, xend = x2, y = y1, yend = y2, 
                             linewidth = abs(Freq), colour = dir), 
               show.legend = F, alpha = 0.2)+
  geom_point(data = l_cond, mapping = aes(x = X1, y = X2, fill = total), 
